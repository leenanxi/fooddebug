Y.use('node', function (Y) {
	window.Site = Singleton.create({
		...
		bindUI: function () {
			...
			this.loadBanner();
			Y.one(window).on('resize', this.loadBanner, this);
		...
		loadBanner: function () {
			...
			var wrapper = Y.one('.collection-type-blog') ?
				Y.one('.sqs-featured-posts-gallery .gallery-wrapper') :
				Y.one('.banner-thumbnail-wrapper');

			var fullscreen = function () {
				if (
					Y.one('.fullscreen-banner-all-pages') ||
					Y.one('.fullscreen-banner-homepage.homepage') ||
					Y.one('.fullscreen-banner-index-pages.collection-type-index')
				) {
					return true;
				} else {
					return false;
			...
			if (fullscreen() === true) {
				wrapper && wrapper.setStyle('height', Y.config.win.innerHeight);
			}

			if (wrapper && wrapper.one('.posts') && wrapper.one('.post')) {
				wrapper.all('.posts, post').each(function (post) {
					post.removeAttribute('style');
				});
			}

			wrapper && wrapper.all('img').each(function (img) {
				ImageLoader.load(wrapper.one('img'), {
					load: true
				});
			});
